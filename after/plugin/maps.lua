...
-- Terminal management with hide/show functionality
local terminal_buffers = {}

local function toggle_terminal(direction, command)
  local key = direction .. (command or "")

  if terminal_buffers[key] and vim.api.nvim_buf_is_valid(terminal_buffers[key].buf) then
    -- Terminal exists, check if it's visible
    local wins = vim.fn.win_findbuf(terminal_buffers[key].buf)
    if #wins > 0 then
